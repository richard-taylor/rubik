...
// Estimate the distribution of optimal solutions to extending a 2x2x2 block
// to a 3x2x2 block on a 3x3x3 cube.
//
// Use a randomised cube that has a solved 2x2x2 block in the RUF corner.
...
int main()
{
    ...
    for (int i = 1; i <= N; i++)
    {
        Cube cube = solved.randomise( { RUF }, { RU, UF, RF } );

        IterativeDeepening solver;
        Block3x2x2 pattern(RUF);
        try
        {
